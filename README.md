...
python -m engine.pipeline reddit --focused --subreddits pennystocks,Shortsqueeze --start 2021-01-01 --end 2021-01-31
...
Writes are idempotent. Focused Reddit backfills use the stored market universe to query one
ticker at a time, verify the ticker text locally, throttle archive requests, and commit progress
incrementally. Dashboard runs are capped at 31 days so history can be built in resumable months.
